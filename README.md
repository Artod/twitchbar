...
- **No twitchbar item in the menu bar, though it is running**: on a MacBook with a notch, macOS hides menu-bar items that do not fit beside the notch when an app with a wide menu is in front. Cmd-drag the twitchbar item out from under the notch once (it stays where you put it), or quit a menu-heavy app. If you started it by running `twitchbar` as the app's own process rather than from `twitchbar.app`, update with `uv tool upgrade twitchbar` and `twitchbar install-app`: the app launcher starts it the way that shows reliably.
